...
def Capacitacion(usuario,puesto):
  ...
  if puesto== "Coordinador":
    ...
    if filtro_8=="Todos":
        data = pd.read_sql(f"select marca,usuario,nombre,fecha,tema,horas,puesto,supervisor from capacitaciones where fecha>='{fecha_de__inicio_8}' and fecha<='{fecha_de__finalizacion_8}'", con)

    elif filtro_8=="Operarios":
        data = pd.read_sql(f"select marca,usuario,nombre,fecha,tema,horas,puesto,supervisor,reporte from capacitaciones where puesto='Operario Catastral' and fecha>='{fecha_de__inicio_8}' and fecha<='{fecha_de__finalizacion_8}'", con)
      
    elif filtro_8=="Propio" :
        data = pd.read_sql(f"select marca,usuario,nombre,fecha,tema,horas,puesto,supervisor,reporte from capacitaciones where usuario='{usuario}' and fecha>='{fecha_de__inicio_8}' and fecha<='{fecha_de__finalizacion_8}'", con)

    elif filtro_8=="Personal Asignado" :
        data = pd.read_sql(f"select marca,usuario,nombre,fecha,tema,horas,puesto,supervisor,reporte from capacitaciones where supervisor='{nombre_8}' and fecha>='{fecha_de__inicio_8}' and fecha<='{fecha_de__finalizacion_8}'", con)
      ...
      if filtro_8=="Todos":
          data = pd.read_sql(f"select marca,usuario,nombre,fecha,tema,horas,puesto,supervisor,reporte from capacitaciones where fecha>='{fecha_de__inicio_8}' and fecha<='{fecha_de__finalizacion_8}'", con)
    ...
    if reporte_8:
      ...
      if personal_8 =='':
        ...
        for nombre in personal_8:
          ...
          cursor01.execute(f"INSERT INTO capacitaciones (marca,usuario,nombre,fecha,tema,horas,puesto,supervisor,reporte)VALUES('{marca_8}','{usuario_8}','{nombre}','{fecha_8}','{tema_8}','{horas_8}','{puesto}','{supervisor_8}','{nombre_8}')")
          con.commit()
