...
def Historial(usuario,puesto):
  ...
  if puesto=="Supervisor" or puesto=="Coordinador":      
    ...
    if filtro_7=="Todos" and proceso_7_s=="Todos":
        data = pd.read_sql(f"select marca,proceso,usuario,nombre,horario,fecha,bloque,estado,predios,horas,puesto,tipo,supervisor from registro where fecha>='{fecha_de__inicio_7}' and fecha<='{fecha_de__finalizacion_7}'", con)
    
    elif filtro_7=="Todos" and proceso_7_s !="Todos":
        data = pd.read_sql(f"select marca,proceso,usuario,nombre,horario,fecha,bloque,estado,predios,horas,puesto,tipo,supervisor from registro where proceso='{proceso_7_s}' and fecha>='{fecha_de__inicio_7}' and fecha<='{fecha_de__finalizacion_7}'", con)
      
    elif filtro_7=="Operarios" and proceso_7_s =="Todos":
        data = pd.read_sql(f"select marca,proceso,usuario,nombre,horario,fecha,bloque,estado,predios,horas,puesto,tipo,supervisor from registro where puesto='Operario Catastral' and fecha>='{fecha_de__inicio_7}' and fecha<='{fecha_de__finalizacion_7}'", con)
    
    elif filtro_7=="Operarios" and proceso_7_s !="Todos":
        data = pd.read_sql(f"select marca,proceso,usuario,nombre,horario,fecha,bloque,estado,predios,horas,puesto,tipo,supervisor from registro where puesto='Operario Catastral' and proceso='{proceso_7_s}' and fecha>='{fecha_de__inicio_7}' and fecha<='{fecha_de__finalizacion_7}'", con)
    
    elif filtro_7=="Propio" and proceso_7_s =="Todos":
        data = pd.read_sql(f"select marca,proceso,usuario,nombre,horario,fecha,bloque,estado,predios,horas,puesto,tipo,supervisor from registro where usuario='{usuario}' and fecha>='{fecha_de__inicio_7}' and fecha<='{fecha_de__finalizacion_7}'", con)

    elif filtro_7=="Propio" and proceso_7_s !="Todos":
        data = pd.read_sql(f"select marca,proceso,usuario,nombre,horario,fecha,bloque,estado,predios,horas,puesto,tipo,supervisor from registro where usuario='{usuario}' and proceso='{proceso_7_s}' and fecha>='{fecha_de__inicio_7}' and fecha<='{fecha_de__finalizacion_7}'", con)

    elif filtro_7=="Personal Asignado" and proceso_7_s =="Todos":
        data = pd.read_sql(f"select marca,proceso,usuario,nombre,horario,fecha,bloque,estado,predios,horas,puesto,tipo,supervisor from registro where supervisor='{nombre_7}' and fecha>='{fecha_de__inicio_7}' and fecha<='{fecha_de__finalizacion_7}'", con)

    elif filtro_7=="Personal Asignado" and proceso_7_s !="Todos":
        data = pd.read_sql(f"select marca,proceso,usuario,nombre,horario,fecha,bloque,estado,predios,horas,puesto,tipo,supervisor from registro where supervisor='{nombre_7}' and proceso='{proceso_7_s}' and fecha>='{fecha_de__inicio_7}' and fecha<='{fecha_de__finalizacion_7}'", con)

  elif puesto=="Operario Catastral":

    placeholder11_7 = st.empty()
    proceso_7_o= placeholder11_7.selectbox("Proceso", options=("Todos","Información Final I","Información Final II","Información Final III"), key="proceso_7_o")
        
    if proceso_7_o =="Todos":
        data = pd.read_sql(f"select marca,proceso,usuario,nombre,horario,fecha,bloque,estado,predios,horas,puesto,tipo,supervisor from registro where usuario='{usuario}' and fecha>='{fecha_de__inicio_7}' and fecha<='{fecha_de__finalizacion_7}'", con)

    else:
        data = pd.read_sql(f"select marca,proceso,usuario,nombre,horario,fecha,bloque,estado,predios,horas,puesto,tipo,supervisor from registro where usuario='{usuario}'and proceso='{proceso_7_o}' and fecha>='{fecha_de__inicio_7}' and fecha<='{fecha_de__finalizacion_7}'", con)
  ...
  nombre_7=data.iloc[:,3]
  fecha_7=data.iloc[:,5]
  predio_7=data.iloc[:,8]
